...
def test_single_site():
    ...
    try:
        ...
        article_links = fetch_article_links(page_url, headers, today, navigation_type)
        
        # Filter articles published today and scrape their content
        print("Filtering and scraping articles...")
        results = filter_and_scrape_articles(article_links, today)
        
        # Generate the output filename with the current date
        today_date_str = today.strftime('%d-%m-%Y')
        output_csv_file = os.path.join(os.path.dirname(__file__), f'scraped_article_{today_date_str}.csv')
