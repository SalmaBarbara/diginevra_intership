...
def navigate_to_date_infinite_scroll(page_url, headers, target_date):
    service = Service(executable_path='scrapingenv\Lib\site-packages\selenium\webdriver\chrome\webdriver.py')  # Update with your chromedriver path
        ...
        try:
            ...
            if article_date:
                ...
                if not isinstance(article_date, datetime):
                    print(f"Unexpected publish_date type for {article_url}: {type(article_date)}")
                    article_date = None
